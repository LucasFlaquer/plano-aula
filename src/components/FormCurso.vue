<template>
  <form @submit.prevent="submit" ref="form" class="form-curso" novalidate>
    ...
    <button class="btn btn-primary">
      {{ cursoId.length > 1 ? 'Atualizar' : 'Adicionar' }}
    </button>
  </form>
</template>
...
export default {
  ...
  methods: {
    ...
    validaForm(form) {
      ...
      campos.forEach((index) => {
        if (!index.checkValidity()) index.parentNode.classList.add('required')
      })
      let invalidos = form.querySelectorAll('.required')
      invalidos[0].focus()
      invalidos.forEach(function (el) {
        el.addEventListener('keyup', function () {
          if (!el.checkValidity) el.classList.remove('required')
        })
  ...
  created() {
    ...
    if (this.cursoId.length > 1) {
      ...
      if (this.curso.coordenador) this.user_id = this.curso.coordenador.id
    }
  },
}
</script>
