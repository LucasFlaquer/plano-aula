<template>
  <Dashboard>
      ...
      <tbody>
        <tr v-for="bibliografia in bibliografias" :key="bibliografia.id">
          ...
          <td>
            ...
            <a
              href="#"
              @click.prevent="deleteBibliografia(bibliografia.id)"
    ...
    <b-modal id="modal-bibliografia" :title="modalTitle" hide-footer>
      <form-bibliografias
...
export default {
  ...
  methods: {
    handleFormOpen(id) {
      ...
      if (id.length > 1) this.modalTitle = 'Editar Bibliografia'
      else this.modalTitle = 'Adicionar Bibliografia'
    },
    handleModalClose() {
